...
В Unix, java устанавливается по умолчанию в директорию usr/lib/jvm/. Смотрим, что у нас находится по этому пути
> $ cd /usr/lib/jvm/; ls
...
Проверим корректность наших действий и выведем наши переменные в консоль:
> $ echo $JAVA_HOME; echo $PATH

Прекрасно, теперь мы можем писать код на java и проверять его в консоли. Для особо пытливых умов - [Установка переменных JAVA_HOME / PATH в Unix/Linux](http://linux-notes.org/ustanovka-peremenny-h-java_home-path-v-linux/).
