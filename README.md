...
Теперь нам нужен пользователь с правами su, работать под root не рекомендую - если накосячить поправить дело не получится - при создании пароль выбирай посложней, чтобы привыкать к работе через internet:
> $ adduser username
...
Теперь можно переподключиться к серверу под новой учетной записью. Завершение сессии можно сделать через ctrl+c. Подключиться к VDS (в будующем будет возможно след командой: `ssh -p 50132 user@77.11.22.33`, где 50132 - порт, а 77.11.22.33 - ip адрес сервера)
