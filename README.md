...
Подключиться к VDS серверу после всех манипуляций можно след командой: `ssh -p 50132 user@77.11.22.33`, где 50132 - порт, а 77.11.22.33 - ip адрес сервера. Сервер запросит пароль указанного `user`а и пропустит.

Подключение по паролю это конечно хорошо, но что делать, если у тебя хороший пароль, а ты такой весь из себя ленивая жопа.

![img](res/fat-cat.jpg)

Весьма запарно вводить длинный пароль каждый раз, к счастью [ssh позволяет более удобно авторизоваться на удаленном сервере](https://habr.com/ru/post/122445/).
