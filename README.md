...
Чтобы пользоваться компилятором на java необходимо прописать переменные окружения для java/bin в __PATH__ и java в __JAVA_HOME__. 
Прежде чем записывать новые переменные, следует удостовериться что они отсутствуют в системе. Проверить это можно пролистав файлы ~/.bash_profile, /etc/environment, /etc/profile на предмет наличия выше указанных переменных. (Просто это можно сделать например через тотже vim). 
